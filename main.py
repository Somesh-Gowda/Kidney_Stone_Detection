...
s = r'KidneyStoneDetection-main\images\final_stone1.jpg'
img = cv2.imread(s, 0)

if img is None:
    raise ValueError(f"Image at path {s} could not be loaded.")
...
def lap(img):
    ...
    return np.abs(result[:inp_shape[0], :inp_shape[1]])
...
def pre_processing(img):
    ...
    img_ostu = filter_extraction(img_rm_small.astype(np.uint8) * 255)
    plt.subplot(2, 2, 2)
